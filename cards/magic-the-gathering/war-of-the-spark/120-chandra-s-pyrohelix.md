---
name: Chandra's Pyrohelix
game: Magic: The Gathering
set: War of the Spark
collector_number: 120
rarity: C
variance: Normal
grade: Ungraded
condition: Near Mint
quantity: 1
held_for_lair: 0
bundles: []
tags_hub: ["fire", "spellcaster", "pyromancer", "robed-figure", "magic", "destruction", "inferno", "back-facing-figure", "silhouette", "explosion", "helix"]
tags_filter: ["solo", "mid-shot", "faces-away", "action-mode", "red-magic", "instant", "no-face-visible"]
reference_image: cards/_images/magic-the-gathering/war-of-the-spark/120-chandra-s-pyrohelix.png
average_cost_paid: 0
market_price: 0.18
market_price_as_of: 2026-05-12
date_added: 2026-05-06
last_seen: 2026-05-12
art_match_confidence: high
needs_manual_review: false
manual_review_reason: 
reference_image_source_url: https://cards.scryfall.io/png/front/1/c/1c482f51-9222-4e9e-a9fd-bb14a0afe156.png?1557576647
mood: action
time_of_day: indeterminate
setting: other
suspected_ip: Chandra Nalaar
ip_confidence: high
ip_verified: true
artist: Aleksi Briclot
art_crop_image: cards/_images/magic-the-gathering/war-of-the-spark/120-chandra-s-pyrohelix--art.jpg
art_crop_source_url: https://cards.scryfall.io/art_crop/front/1/c/1c482f51-9222-4e9e-a9fd-bb14a0afe156.jpg?1557576647
flavor_text: \"Now that I've dealt with your little toys, I can focus on the real problem: you.\"\n—Chandra Nalaar, to Dovin Baan
oracle_text: Chandra's Pyrohelix deals 2 damage divided as you choose among one or two targets.
---

# Chandra's Pyrohelix (War of the Spark)

## Vision

![120-chandra-s-pyrohelix](../../_images/magic-the-gathering/war-of-the-spark/120-chandra-s-pyrohelix.png)

> [!note] IP verified: **Chandra Nalaar**
> Confirmed via Scryfall (`flavor_text` attributes the quote to Chandra Nalaar) and MTG Wiki [Chandra Nalaar].

A back-facing figure in a long dark coat or robe stands at the center of the frame, arms extended forward, channeling two intertwined helical streams of fire into a vast roaring blaze that consumes the background. The composition is dominated by saturated oranges, yellows, and deep reds, with the silhouette of the caster reading almost black against the firelight. Embers and motion-blurred flame curl around the edges of the frame. The pose is dynamic and forward-thrusting, conveying an explosive offensive spell mid-cast.

**Subject:** A robed figure with long hair, viewed from behind, hurling twin spiraling helices of fire forward into a blazing inferno

**Composition:** mid-shot, action, figures: solo, facing: away
**Setting:** other, indeterminate, fire
**Foreground:** back-facing robed caster with arms outstretched, twin fire helices erupting forward  *(palette: black, dark-red, orange)*
**Background:** wall of roaring flame and embers consuming the scene  *(palette: orange, yellow, red, ember-orange)*
**Mood / lighting:** action, backlit
**Emotion read:** aggressive, focused, mid-cast intensity
**Objects:** fire-helix, flames, embers
**Iconography:** double-helix, spiral, fire-blast
**Genre cues:** fantasy, high-fantasy

## Trivia

- **Set context** — Released in *War of the Spark* (WAR) on 2019-05-03; the set centers on Nicol Bolas's Eternal army invading Ravnica with all 36 planeswalkers featured. Chandra was a frontline Gatewatch combatant in the conflict `[Scryfall API: set page]`.
- **Design** — Art by **Aleksi Briclot**, one of Chandra's signature artists — Briclot also painted the original 2007 *Chandra Nalaar* planeswalker (Lorwyn) and *Chandra's Phoenix* (M12), giving this common-slot spell unusual portrait-grade pedigree `[Scryfall API: artist credit]`.
- **Mechanical reputation** — Tracked in **6,569 Commander decks** on EDHREC, with top synergy partners including *Renegade Firebrand*, *Chandra's Revolution*, and *Furious Reprisal* — a budget red split-damage instant whose appeal is target flexibility (2-to-one or 1-and-1) at common rarity `[EDHREC: Chandra's Pyrohelix card page]`.
- **Flavor text** — *"Now that I've dealt with your little toys, I can focus on the real problem: you."* — Chandra Nalaar, to Dovin Baan `[Scryfall flavor_text]`. Dovin Baan is the Kaladesh-born planeswalker who defected to Bolas and engineered the Thopter constructs — the "little toys" the quote dismisses `[MTG Wiki: Chandra Nalaar; War of the Spark set narrative context]`.
- **Cycle context** — Part of the *Chandra's <Spell>* flavor cycle, currently spanning **17 cards** on Scryfall (incl. *Chandra's Outrage, Ignition, Phoenix, Embercat, Firemaw, Defeat, Spitfire, Triumph, Revolution, Magmutt, Pyreling, Regulator, Flame Wave, Fury, Incinerator*), all canonically depicting or attributed to Chandra Nalaar's pyromancy `[Scryfall search: name:"Chandra's"]`.

### Related cards
- *Chandra Nalaar* (Lorwyn LRW, plus many reprints) — original 2007 planeswalker depiction of Chandra; same artist (Aleksi Briclot).
- *Chandra's Phoenix* (Magic 2012, M12) — sibling Briclot-illustrated card in the Chandra-attributed spell cycle.
- *Chandra's Revolution* (Aether Revolt, AER) — EDHREC names it as a frequent co-deck inclusion with this card.
- *Renegade Firebrand* (Aether Revolt, AER) — top EDHREC synergy partner per the Pyrohelix card page.
- *Furious Reprisal* (multiple printings) — direct mechanical cousin: red instant dealing 2 damage divided between targets; cited by EDHREC as a play-pattern relative.
- *Dovin's Veto* (War of the Spark, WAR) — companion-cycle counterpart for Dovin Baan, the flavor-text target named in this card's quote.

