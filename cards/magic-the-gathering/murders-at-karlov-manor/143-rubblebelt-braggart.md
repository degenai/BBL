---
name: Rubblebelt Braggart
game: Magic: The Gathering
set: Murders at Karlov Manor
collector_number: 143
rarity: C
variance: Normal
grade: Ungraded
condition: Near Mint
quantity: 4
held_for_lair: 0
bundles: []
tags_hub: ["monster", "braggart", "bone", "skull", "horns", "aggression", "roar", "warrior", "brute", "forest", "trophy"]
tags_filter: ["duo", "mid-shot", "action", "forward-facing", "creature"]
reference_image: cards/_images/magic-the-gathering/murders-at-karlov-manor/143-rubblebelt-braggart.png
average_cost_paid: 0
market_price: 0.05
market_price_as_of: 2026-05-14
date_added: 2026-05-14
last_seen: 2026-05-14
reference_image_source_url: https://cards.scryfall.io/png/front/f/9/f90f8691-210a-4bf0-9fc2-fb2efcf057fb.png?1706241918
art_match_confidence: high
needs_manual_review: false
artist: Leonardo Santanna
art_crop_image: cards/_images/magic-the-gathering/murders-at-karlov-manor/143-rubblebelt-braggart--art.jpg
art_crop_source_url: https://cards.scryfall.io/art_crop/front/f/9/f90f8691-210a-4bf0-9fc2-fb2efcf057fb.jpg?1706241918
flavor_text: \"I did it! Double—no, triple homicide! In a building I set on fire. Yep, all me.\"
oracle_text: Whenever this creature attacks, if it's not suspected, you may suspect it. (A suspected creature has menace and can't block.)
image_width: 745
image_height: 1040
image_quality: high
mood: action
time_of_day: day
setting: forest
---

# Rubblebelt Braggart (Murders at Karlov Manor)

## Vision

![143-rubblebelt-braggart](../../_images/magic-the-gathering/murders-at-karlov-manor/143-rubblebelt-braggart.png)

A massive creature with distinctly red skin, multiple large horns, and a wide-open mouth mid-roar fills the foreground. Its body is covered in a patchwork of bone-decorated armor — plates, chains, and what appear to be carved bone pieces assembled into a rough but substantial kit. The creature lunges forward aggressively with one arm extended toward the viewer, fist clenched. A small skull ornament is visible on its gear. In the background, a pale or bone-colored creature (possibly skeletal or lightly-colored animal) is visible in a forested setting. The environment suggests an overgrown ruin or wild territory. The creature's posture and open-mouthed expression convey boastful aggression.

**Subject:** A large red-skinned horned creature in bone-decorated armor lunging forward with a roar, one fist raised toward the viewer

**Composition:** mid-shot, action, figures: duo, facing: forward
**Setting:** forest, ruined, day, calm
**Foreground:** red-skinned horned creature in bone armor lunging forward  *(palette: red-orange, bone-white, brown, tan)*
**Background:** overgrown forest, pale creature in background  *(palette: green, pale-grey, muted-brown)*
**Mood / lighting:** action, soft
**Emotion read:** roaring boastful aggression — chest-puffed bravado
**Objects:** bone armor, skull ornament, horns
**Creatures:** horned creature, pale background creature
**Iconography:** skull
**Genre cues:** fantasy

## Trivia

- **Set context** — Rubblebelt Braggart is a red 5/5 Creature with type Lizard Warrior. The Rubblebelt is Ravnica's discontinuous wasteland zone held by the Gruul Clans — overgrown ruins on the fringes of the Tenth District where guild order breaks down. Lizard Warriors are an established Gruul-adjacent creature type in Ravnica, with Rubblebelt Runner from Ravnica Allegiance as a direct predecessor `[Scryfall, WebSearch: mtg.wiki/page/Gruul_Clans]`.
- **Mechanical design** — The Suspect keyword is MKM's murder-mystery mechanic: a suspected creature gains menace and cannot block. Rubblebelt Braggart self-suspects on attack (if not already suspected), trading its blocking ability for menace — a strong trade for a 5/5 that has no need to block `[Scryfall oracle text, magic.wizards.com/en/news/feature/murders-at-karlov-manor-mechanics]`.
- **Flavor text** — "I did it! Double — no, triple homicide! In a building I set on fire. Yep, all me." — Speaker implied: the Braggart `[Scryfall]`. The escalating self-correction (double to triple, mid-sentence) is the joke: bragging about crimes as social currency, inflating the count in real time. It is also the most literally descriptive Suspect-mechanic flavor text in MKM.
- **Design** — Artist: Leonardo Santanna. Previewed via the official Wizards card gallery on 2024-01-23. EDHREC rank 25895 — low Commander uptake, consistent with a 5-mana common facing stiff competition `[Scryfall]`.

### Related cards
- Rubblebelt Runner (Ravnica Allegiance, no. 202) — earlier Lizard Warrior from the Rubblebelt; establishes the creature type's MKM lineage
- Person of Interest (MKM, no. 139) — second Suspect-keyword card confirmed in this batch alongside Braggart
- Skarrgan Hellkite (Ravnica Allegiance) — high-power red Gruul-adjacent creature from Ravnica; thematic comparison at the top of the curve

