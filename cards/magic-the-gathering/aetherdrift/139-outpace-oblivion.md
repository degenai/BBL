---
name: Outpace Oblivion
game: Magic: The Gathering
set: Aetherdrift
collector_number: 139
rarity: U
variance: Normal
grade: Ungraded
condition: Near Mint
quantity: 2
held_for_lair: 0
bundles: []
tags_hub: ["racer", "racing", "vehicle", "goggles", "explosion", "fire", "escape", "chase", "speed", "rider", "pyromancy", "destruction"]
tags_filter: ["red-magic", "enchantment", "single-figure-focal", "group", "close-up", "three-quarter-facing", "action-mode", "female-figure", "forward-motion", "raymond-swanland"]
reference_image: cards/_images/magic-the-gathering/aetherdrift/139-outpace-oblivion.png
average_cost_paid: 0
market_price: 0.05
market_price_as_of: 2026-05-14
date_added: 2026-05-11
last_seen: 2026-05-14
reference_image_source_url: https://cards.scryfall.io/png/front/c/2/c22e415f-636f-4394-9b21-600ab720ac98.png?1738356442
art_match_confidence: high
needs_manual_review: false
artist: Raymond Swanland
art_crop_image: cards/_images/magic-the-gathering/aetherdrift/139-outpace-oblivion--art.jpg
art_crop_source_url: https://cards.scryfall.io/art_crop/front/c/2/c22e415f-636f-4394-9b21-600ab720ac98.jpg?1738356442
oracle_text: Start your engines! (If you have no speed, it starts at 1. It increases once on each of your turns when an opponent loses life. Max speed is 4.)\nWhen this enchantment enters, it deals 5 damage to up to one target creature or planeswalker.\n{2}, Sacrifice this enchantment: It deals 2 damage to each player who doesn't have max speed.
mood: action
time_of_day: indeterminate
setting: other
---

# Outpace Oblivion (Aetherdrift)

## Vision

![139-outpace-oblivion](../../_images/magic-the-gathering/aetherdrift/139-outpace-oblivion.png)

Dynamic forward-thrust composition. A red-haired racer in red-tinted goggles, plated shoulder pauldron, and bracered gauntlet leans low into a turn on a curving cyan speed-track that arcs left into the distance. The figure dominates the right half of the frame, head turned partially back toward the viewer while the body angles forward. Behind them an enormous orange-and-yellow fireball blossoms, silhouetting a second rider mid-immolation and several airborne bird-like shapes. Jagged dark spires and metallic claw-like architecture frame the explosion. The track's blue glow rim-lights the racer's face and goggle lenses against the hot reds and oranges of the blast. High-contrast Swanland palette: cyan track versus furnace orange.

**Subject:** A goggled racer crouched forward on a vehicle, hair streaming back, glancing over the shoulder as a fireball erupts behind them on a glowing speed-track; a flaming pursuer and birds are caught in the blast

**Composition:** close-up, action, figures: group, facing: three-quarter
**Setting:** other, ruined, indeterminate, fire
**Foreground:** goggled racer in armored riding gear, leaning over handlebar with gauntleted arm  *(palette: red, orange, steel-gray, cyan, black)*
**Background:** explosive fireball engulfing a pursuing rider on a curving glowing race track, with jagged spires and airborne birds  *(palette: orange, yellow, red, black, cyan)*
**Mood / lighting:** action, rim
**Emotion read:** focused, defiant, narrowly escaping
**Objects:** goggles, vehicle, handlebar, gauntlet, pauldron, speed-track, fireball
**Creatures:** birds
**Iconography:** goggles, speed-track, explosion
**Genre cues:** fantasy, sci-fi, post-apocalyptic, racing

## Trivia

- **Set context** — Aetherdrift (DFT) released February 14, 2025, as MTG's 103rd expansion. The set stages an interplanar death-race across Avishkar (formerly Kaladesh — renamed by Wizards in consultation with cultural advisors to remove racial connotations from the Hindi-derived original name), Amonkhet, and Muraganda. The race's prize is the Aetherspark, an artificial planeswalker spark built by an Avishkari inventor during the New Phyrexian Invasion `[magic.wizards.com: Planeswalker's Guide to Aetherdrift Part 1; Scryfall: DFT set metadata]`.
- **Mechanical** — Outpace Oblivion carries Aetherdrift's core keyword "Start your engines!" — speed begins at 1 on first trigger and increases by 1 on each of your turns when an opponent loses life, capping at 4 (max speed). The card deals 5 damage on entry and can sacrifice itself to deal 2 damage to each player not at max speed, punishing opponents who fall behind the acceleration curve. No flavor text `[Scryfall: card c22e415f]`.
- **Design** — Art by Raymond Swanland, a veteran Magic illustrator known for high-contrast, cinematic action compositions. The card was previewed January 30, 2025, via the official Wizards of the Coast YouTube channel. EDHREC rank 16,838 reflects limited Commander demand; the card's design is oriented toward Standard and speed-focused aggro builds rather than singleton formats `[Scryfall: card c22e415f — artist, edhrec_rank, preview fields]`.

### Related cards
- Start Your Engines! // Max Speed (Aetherdrift) — Scryfall marks this as a direct combo_piece partner for Outpace Oblivion; it's the double-faced card representing the speed mechanic itself
- Pacesetter Paragon (Aetherdrift, no. 140) — same set, same Red color, consecutive collector number; shares the racing-pilot theme
- Alacrian Jaguar (Aetherdrift, no. 152) — same set, Green, the Mount-based counterpart to the Red racer cards in the DFT lineup

