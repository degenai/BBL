---
name: Valakut Invoker
game: "Magic: The Gathering"
set: Battle for Zendikar
collector_number: 159
rarity: C
variance: Normal
grade: Ungraded
condition: Near Mint
quantity: 1
held_for_lair: 0
bundles: []
tags_hub:
  - fire
  - channeling
  - magic-user
  - volcano
  - power
  - ritual
  - close-up-portrait
  - warrior
  - invoker
  - heat
tags_filter:
  - solo
  - close-up
  - three-quarter
  - action
  - red-magic
  - creature-human-shaman
reference_image: cards/_images/magic-the-gathering/battle-for-zendikar/159-valakut-invoker.png
average_cost_paid: 0
market_price: 0.05
market_price_as_of: 2026-05-16
date_added: 2026-05-14
last_seen: 2026-05-16
art_match_confidence: high
needs_manual_review: false
manual_review_reason: No reference image found via scryfall for set 'Battle for Zendikar'
reference_image_source_url: https://cards.scryfall.io/png/front/1/7/178c66b8-c577-49c6-9efc-7bab740e66b6.png?1562900685
artist: Joseph Meehan
art_crop_image: cards/_images/magic-the-gathering/battle-for-zendikar/159-valakut-invoker--art.jpg
art_crop_source_url: https://cards.scryfall.io/art_crop/front/1/7/178c66b8-c577-49c6-9efc-7bab740e66b6.jpg?1562900685
flavor_text: \"As long as we have walked this world, we have known it as a harsh and unforgiving place.\"\n—*The Invokers' Tales*
oracle_text: "{8}: This creature deals 3 damage to any target."
image_width: 745
image_height: 1040
image_quality: high
mood: action
time_of_day: indeterminate
setting: mountain
tags:
  - card
  - vision-passed
  - trivia-passed
---
# Valakut Invoker (Battle for Zendikar)

## Vision

![159-valakut-invoker](../../_images/magic-the-gathering/battle-for-zendikar/159-valakut-invoker.png)

A humanoid figure fills the frame in a close portrait crop, head tilted back with eyes closed or near-closed, suggesting concentration or ecstasy in channeling. A glowing sphere of fire energy is visible at the figure's chest, held between the hands. The figure wears dark clothing with what appears to be a bandolier or armored vest. The background shows orange-red rocky volcanic walls. The lighting is entirely warm — deep red and orange — casting the face in dramatic chiaroscuro.

**Subject:** A humanoid figure in close-up, head tilted back, channeling a glowing fire orb held at the chest, in a volcanic rocky environment

**Composition:** close-up, action, figures: solo, facing: three-quarter
**Setting:** mountain, indeterminate, fire
**Foreground:** humanoid figure channeling fire orb at chest, head back  *(palette: orange, red, brown)*
**Background:** volcanic rock walls, warm orange-red glow  *(palette: orange, red, dark-brown)*
**Mood / lighting:** action, chiaroscuro
**Emotion read:** concentration, power-channeling, barely-contained intensity
**Objects:** fire orb
**Iconography:** fire orb, channeling gesture
**Genre cues:** fantasy

## Trivia

- **Design: Invoker pattern** — Valakut Invoker belongs to the Invoker design category: common creatures with a repeatable {8}-mana activated ability intended as a late-game mana sink. The pattern originated in Legions (2003), returned in Rise of the Eldrazi (2010), and appeared as only two cards in BFZ (Valakut Invoker red, and Oran-Rief Invoker green, 182) rather than a full 5-color cycle. `[MTG Wiki: Invoker, mtg.wiki/page/Invoker]`
- **Design: partial cycle** — BFZ fielded just two Invokers rather than a full cycle, a compression apparently driven by the set's Eldrazi-invasion design focus. The pattern returned to a full cycle in Commander Legends: Battle for Baldur's Gate (2022). `[MTG Wiki: Invoker, mtg.wiki/page/Invoker]`
- **Flavor text** — `"As long as we have walked this world, we have known it as a harsh and unforgiving place."` — *The Invokers' Tales* `[Scryfall]`. The source is a named in-world text rather than a named NPC — one of the few BFZ flavor credits citing a fictional document. The quote implies an oral/written tradition of invoker knowledge passed down through Zendikar's history.
- **Mechanical profile** — 2/3 for {2}{R}, {8}: deals 3 damage to any target. Legal in Pauper (common), Pioneer, Modern, Legacy, and Commander. EDHREC rank 15315. The {8} cost is intentional to the Invoker template — useful only in games that go long enough to accumulate that mana, functioning as a built-in late-game topdeck value mode on an otherwise vanilla-statted body. `[Scryfall]`

### Related cards
- Oran-Rief Invoker (Battle for Zendikar, 182) — BFZ pair-mate; green, gives +5/+5 and trample for {8}
- Valakut, the Molten Pinnacle (Zendikar, 228) — the volcanic landmark this invoker is named for; original Zendikar 2009
- Retreat to Valakut (Battle for Zendikar, 153) — second BFZ card named for Valakut; shares the red/volcanic location
- Flamekin Invoker (Lorwyn) — earlier red Invoker cycle member; same {8}-mana design template

