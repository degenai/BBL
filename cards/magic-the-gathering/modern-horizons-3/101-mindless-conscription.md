---
name: Mindless Conscription
game: Magic: The Gathering
set: Modern Horizons 3
collector_number: 101
rarity: U
variance: Normal
grade: Ungraded
condition: Near Mint
quantity: 2
held_for_lair: 0
bundles: []
tags_hub: ["zombie", "undead", "horde", "army", "necromancy", "dark-ritual", "mass-death", "obelisk", "conscription", "dread", "apocalypse"]
tags_filter: ["crowd", "wide", "scene-mode", "black-magic", "enchantment", "amass"]
reference_image: cards/_images/magic-the-gathering/modern-horizons-3/101-mindless-conscription.png
average_cost_paid: 0
market_price: 0.19
market_price_as_of: 2026-05-14
date_added: 2026-05-14
last_seen: 2026-05-14
reference_image_source_url: https://cards.scryfall.io/png/front/1/a/1af5b195-101a-4265-98a7-522a968cf218.png?1717012023
art_match_confidence: high
needs_manual_review: false
artist: Warren Mahy
art_crop_image: cards/_images/magic-the-gathering/modern-horizons-3/101-mindless-conscription--art.jpg
art_crop_source_url: https://cards.scryfall.io/art_crop/front/1/a/1af5b195-101a-4265-98a7-522a968cf218.jpg?1717012023
oracle_text: When this enchantment enters and whenever you draw your third card each turn, amass Zombies 3. (Put three +1/+1 counters on an Army you control. It's also a Zombie. If you don't control an Army, create a 0/0 black Zombie Army creature token first.)
image_width: 745
image_height: 1040
image_quality: high
mood: grim
time_of_day: night
setting: other
---

# Mindless Conscription (Modern Horizons 3)

## Vision

![101-mindless-conscription](../../_images/magic-the-gathering/modern-horizons-3/101-mindless-conscription.png)

A wide landscape showing a crowd of shambling undead figures under a murky blue-purple sky. At the center-left rises a massive dark formation that appears to be a twisted column or obelisk-like structure composed of organic or necrotic material. In the foreground left, a large imposing zombie figure walks toward the viewer with long clawed arms hanging low. On the right, another undead figure reaches outward with outstretched arms. Further back, dozens of smaller figures converge on the central structure. The ground is barren and cracked, with wispy purple-magenta energy trailing from some figures. Distant birds or shapes appear high in the sky. The atmosphere is that of a mass conscription or summoning — many dead moving with purposeless inevitability.

**Subject:** A vast undead horde converging on a dark twisted obelisk or mass formation, with zombie figures in deep blue and purple fog

**Composition:** wide, narrative, figures: crowd, facing: three-quarter
**Setting:** other, night, fog
**Foreground:** Large zombie figure with long arms on left, another reaching undead figure on right, barren cracked earth with purple energy wisps  *(palette: blue-grey, purple, magenta, dark brown)*
**Background:** Twisted dark obelisk or mass formation, crowd of undead figures, murky blue sky with distant birds  *(palette: deep blue, navy, dark grey, purple)*
**Mood / lighting:** grim, ambient
**Emotion read:** mindless convergence, purposeless dread
**Objects:** obelisk, cracked earth
**Creatures:** zombie, undead horde
**Iconography:** purple energy wisps, twisted mass formation
**Genre cues:** fantasy, horror

## Trivia

- **Set context** — Modern Horizons 3 (set code MH3, released 2024-06-14) is a direct-to-Modern expansion with no single-plane identity; its cards draw on mechanics and characters from across the game's full history. MH3's black archetype leans on draw-matters and graveyard themes, with Amass Zombies as a payoff keyword `[Scryfall API]`.
- **Mechanical design** — Amass Zombies (as distinct from Amass Orcs introduced in The Lord of the Rings: Tales of Middle-earth) specifies the Army token's subtype as Zombie — relevant for Zombie-tribal payoffs. The draw-third-card trigger rewards blue-black shells with cantrip density; playing this card on turn 3 in a draw-heavy build can amass immediately on entry then chain again within the same turn if a third card is drawn `[Scryfall]`.
- **Format profile** — Legal in Modern, Legacy, Vintage, Commander, and Historic (not Standard or Pioneer). No flavor text (Scryfall flavor_text field is null). EDHREC rank 12,603 suggests modest Commander play — likely in Zombie-tribal or draw-heavy black builds `[Scryfall]`.
- **Design** — Illustrated by Warren Mahy, a New Zealand-based artist whose MTG portfolio spans Zendikar, Innistrad, and Theros blocks, known for atmospheric wide-angle creature and landscape work `[Scryfall]`.

### Related cards
- Liliana, the Last Hope (Eldritch Moon) — Zombie tribal commander-level card; Mindless Conscription feeds her ultimate's zombie accumulation
- Stitcher Geralf (Commander 2014) — blue-black zombie-army commander; canonical shell for the draw-third-trigger
- Army of the Damned (Innistrad) — mass zombie summoning enchantment; thematic ancestor of the zombie-army accumulation pattern
- Cryptbreaker (Eldritch Moon) — zombie-draw engine; creates the draw chain that fuels Mindless Conscription's recurring trigger

