---
name: Daybreak Chimera
game: "Magic: The Gathering"
set: Theros Beyond Death
collector_number: 10
rarity: C
variance: Normal
grade: Ungraded
condition: Near Mint
quantity: 2
held_for_lair: 0
bundles: []
tags_hub:
  - chimera
  - wings
  - flight
  - sky
  - horns
  - goat
  - dawn
  - daybreak
  - celestial
  - mythological-beast
  - greek-myth
  - ascension
tags_filter:
  - solo
  - mid-shot
  - three-quarter-facing
  - action-mode
  - no-face-human
  - creature-chimera
  - white-magic
  - flying
  - devotion-white
reference_image: cards/_images/magic-the-gathering/theros-beyond-death/10-daybreak-chimera.png
average_cost_paid: 0
market_price: 0.09
market_price_as_of: 2026-05-27
date_added: 2026-05-06
last_seen: 2026-05-26
art_match_confidence: high
needs_manual_review: false
manual_review_reason:
reference_image_source_url: https://cards.scryfall.io/png/front/f/9/f9c1de31-73d4-4c32-9f99-e15df48638e9.png?1581479004
mood: sublime
time_of_day: day
setting: other
artist: Lars Grant-West
characters:
  - theros-pantheon
  - heliod
art_crop_image: cards/_images/magic-the-gathering/theros-beyond-death/10-daybreak-chimera--art.jpg
art_crop_source_url: https://cards.scryfall.io/art_crop/front/f/9/f9c1de31-73d4-4c32-9f99-e15df48638e9.jpg?1581479004
oracle_text: This spell costs {X} less to cast, where X is your devotion to white. (Each {W} in the mana costs of permanents you control counts toward your devotion to white.)\nFlying
mana_cost: "{3}{W}{W}"
image_width: 745
image_height: 1040
image_quality: high
tags:
  - card
  - vision-passed
  - trivia-passed
---
# Daybreak Chimera (Theros Beyond Death)

## Vision

![10-daybreak-chimera](../../_images/magic-the-gathering/theros-beyond-death/10-daybreak-chimera.png)

A radiant white chimera ascends through a luminous pale-blue sky. The creature blends a horned, goat-like head and forequarters with massive feathered wings spread wide in flight. Its coat and plumage are uniformly bright white, lit from above so the wings almost dissolve into the sky. The composition is dynamic and lifted — the body angles upward and to the right, wings catching the light. The background is an open expanse of soft blue and white, evoking dawn or high-altitude day. The overall mood is heroic, celestial, and clean — a classic white-magic flying creature presented in mid-action against open sky.

**Subject:** A white-furred winged chimera with goat-like horned head and outstretched feathered wings, soaring through a bright sky

**Composition:** mid-shot, action, figures: solo, facing: three-quarter
**Setting:** other, day, clear
**Foreground:** horned white winged chimera in flight  *(palette: white, ivory, pale-grey)*
**Background:** open bright sky with soft cloud haze  *(palette: pale-blue, white, soft-cyan)*
**Mood / lighting:** sublime, backlit
**Emotion read:** soaring, ascendant, alert
**Creatures:** chimera, goat, winged-beast
**Iconography:** wings, horns
**Genre cues:** fantasy, mythological, greek-myth

## Trivia

- **Set context** — Theros Beyond Death (THB, January 24 2020) is the 84th Magic expansion, returning to the Greek-mythos plane of Theros with a focus on the Underworld and Elspeth's death-and-resurrection arc. THB introduced the Escape mechanic (cast from graveyard by exiling other cards) and returned the Constellation enchantment-payoff trigger from the original Theros block `[Scryfall]`.
- **Mechanical note** — Daybreak Chimera is a 3/3 flyer with base cost {3}{W}{W} that reduces by your devotion to white — effectively free in a white-heavy board. The devotion mechanic first appeared in the original Theros (2013) and returned here as a full-set throughline, treating faith in the Therosian gods as a measurable, game-altering resource `[Scryfall]`.
- **Design reception** — Rated B by Hipsters of the Coast's THB common review: "A 3/3 flying for five doesn't cut it anymore...but Daybreak Chimera looks awesome," noting that casting it alongside another spell for two mana made it a strong tempo play in sealed and draft `[Hipsters of the Coast: "Every Common in Theros Beyond Death Reviewed"]`.
- **Lore context** — On Theros, chimeras are multi-part creatures considered children of Nyx, the divine dreamscape that manifests the gods' will. Unlike the chimeras of real-world myth (lion/goat/serpent), Therosian chimeras take varied beast-hybrid forms; this one's goat-and-eagle body echoes Theros's white-aligned connection to the heavens and Heliod's sun-domain `[Scryfall type_line: Creature — Chimera]`.

### Related cards
- Loathsome Chimera (THB, no. 157) — another THB chimera sharing the Chimera creature type within the same set
- Mischievous Chimera (THB, no. 61) — blue-aligned THB chimera, part of the Flash-matters archetype for the set
- Treeshaker Chimera (THB, no. 184) — green THB chimera; together the three form a cross-color chimera presence across the set
- Heliod, Sun-Crowned (THB, no. 18) — white devotion payoff god; Daybreak Chimera's devotion mechanic directly feeds Heliod's power and toughness as a deity `[Scryfall]`

## Connections

- [[heliod]] — White-aligned ascending chimera whose goat-and-eagle iconography is canonically attributed to Heliod's sun-domain via trivia (*"Theros's white-aligned connection to the heavens and Heliod's sun-domain"*) and Related cards (*"Heliod, Sun-Crowned (THB, no. 18) — white devotion payoff god; Daybreak Chimera's devotion mechanic directly feeds Heliod's power and toughness as a deity"*). The card mechanizes white-devotion-cost-reduction at oracle-text level — the rules-text rendering of devotion-to-Heliod where more {W} pips in your board make sun-cult magic cheaper and stronger. Domain-attribution at the same evidence-tier as Kragma Warcaller THS-195 on `mogis` and Grim Physician THB-100 on `erebos`: god not flavor-named, but the depicted figure's domain-affiliation is Wizards-canonical via MTG Wiki Heliod and the white-devotion-mechanic identity. Card also attaches at the parent `theros-pantheon` cohort via frontmatter — same dual-membership pattern as the `mogis` / `nylea` / `iroas` / `erebos` sub-node cards. `[Scryfall: thb/10; MTG Wiki: Heliod, God of the Sun; MTG Wiki: Heliod, Sun-Crowned]`
