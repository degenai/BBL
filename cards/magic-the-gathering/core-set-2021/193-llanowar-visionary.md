---
name: Llanowar Visionary
game: Magic: The Gathering
set: Core Set 2021
collector_number: 193
rarity: C
variance: Normal
grade: Ungraded
condition: Near Mint
quantity: 2
held_for_lair: 0
bundles: []
tags_hub: ["elf", "druid", "forest", "magic-casting", "nature-magic", "ritual", "glowing-eyes", "bone-armor", "long-hair", "wilderness", "spellcaster", "robed-figure"]
tags_filter: ["solo", "mid-shot", "forward-facing", "action-mode", "creature-elf", "creature-druid", "green-magic", "female-figure", "card-draw"]
reference_image: images/magic-the-gathering/core-set-2021/193-llanowar-visionary.png
average_cost_paid: 0
market_price: 0.28
market_price_as_of: 2026-05-07
date_added: 2026-05-06
last_seen: 2026-05-07
reference_image_source_url: https://cards.scryfall.io/png/front/d/6/d6e23afa-7e08-4049-baf0-d4d0134ba2c8.png?1594737093
art_match_confidence: high
needs_manual_review: false
manual_review_reason: No reference image found via scryfall for set 'Core Set 2021'
mood: sublime
time_of_day: day
setting: forest
---

# Llanowar Visionary (Core Set 2021)

## Vision

![[images/magic-the-gathering/core-set-2021/193-llanowar-visionary.png]]

A pale-skinned elf druid stands at center frame in a dense, sun-dappled forest, facing forward with both arms raised in a casting gesture. Glowing tendrils of green energy spiral outward from the hands and torso, weaving through the air in luminous coils. The figure wears layered organic armor — pauldrons resembling claws or talons of bone-bark, a fitted dark chestplate with vine and twig motifs, and a slight ornament across the brow. Long dark hair frames the face; the eyes carry a faint glow. Behind the druid, vertical tree trunks recede into deeper shadow on either side, with bright forest light filtering between them. The palette is dominated by emerald and lime greens with warm golden highlights and deep forest shadow.

**Subject:** An elf druid standing in a dense forest, channeling swirling green magical energy with both raised hands. The figure has long dark hair, sharply pointed ears, and wears bone-and-bark ornamentation — claw-like shoulder pieces and a layered woody chestplate. Eyes glow with pale magical light.

**Composition:** mid-shot, action, figures: solo, facing: forward
**Setting:** forest, day, calm
**Foreground:** elf druid casting, swirling green energy tendrils  *(palette: emerald-green, lime-green, bone-white, dark-brown, pale-skin)*
**Background:** dense forest with vertical tree trunks, sun-dappled light filtering through canopy  *(palette: forest-green, moss-green, golden-yellow, deep-shadow, olive)*
**Mood / lighting:** sublime, backlit
**Emotion read:** focused, entranced — concentration in the act of conjuring; serene confidence
**Objects:** bone-armor, bark-armor, magical-tendrils, tree-trunks
**Creatures:** elf
**Iconography:** spiral-energy, glowing-eyes, raised-hands-casting
**Genre cues:** fantasy, high-fantasy, druidic

