---
name: Sanctum of Stone Fangs
game: Magic: The Gathering
set: Core Set 2021
collector_number: 120
rarity: U
variance: Normal
grade: Ungraded
condition: Near Mint
quantity: 1
held_for_lair: 0
bundles: []
tags_hub: ["cave", "underground", "stalactites", "shrine", "fangs", "darkness", "stone", "subterranean", "sacred-space", "monolithic", "cathedral", "no-figures"]
tags_filter: ["scene-mode", "wide-shot", "no-face", "no-figure", "enchantment", "legendary-enchantment", "shrine-type", "black-magic", "landscape-art"]
reference_image: cards/_images/magic-the-gathering/core-set-2021/120-sanctum-of-stone-fangs.png
average_cost_paid: 0
market_price: 1.69
market_price_as_of: 2026-05-14
date_added: 2026-05-06
last_seen: 2026-05-14
art_match_confidence: high
needs_manual_review: false
manual_review_reason: 
reference_image_source_url: https://cards.scryfall.io/png/front/d/1/d1439412-618c-483d-89b9-5ea37f0f1edc.png?1594736357
mood: grim
time_of_day: indeterminate
setting: dungeon
artist: Johannes Voss
art_crop_image: cards/_images/magic-the-gathering/core-set-2021/120-sanctum-of-stone-fangs--art.jpg
art_crop_source_url: https://cards.scryfall.io/art_crop/front/d/1/d1439412-618c-483d-89b9-5ea37f0f1edc.jpg?1594736357
flavor_text: Renounce all limits,\nFalse chains binding potential.\nPower knows no bounds.
oracle_text: At the beginning of your first main phase, each opponent loses X life and you gain X life, where X is the number of Shrines you control.
---

# Sanctum of Stone Fangs (Core Set 2021)

## Vision

![120-sanctum-of-stone-fangs](../../_images/magic-the-gathering/core-set-2021/120-sanctum-of-stone-fangs.png)

A cathedral-scale cave interior dominated by towering stalactites and stalagmites that meet and interlock like fangs in a closing jaw. The chamber is lit from a hidden source somewhere deep within, casting a cool teal-green glow that picks out the wet, mineral-slick surfaces of the rock formations. The foreground edges are nearly black silhouette, framing the receding throat of the cave. No figures are present — the space itself is the subject, vast and predatory in its geometry. The mood is sacred and ominous, a shrine carved by geology rather than hands.

**Subject:** A vast subterranean cavern lined with massive stone teeth — stalactites descending from above and stalagmites rising from below, forming a fanged maw of rock around an empty central space

**Composition:** wide, narrative, figures: none, facing: n/a
**Setting:** dungeon, organic, indeterminate
**Foreground:** Dark silhouetted stalactites and stalagmites framing the cave mouth, jagged stone teeth in near-black  *(palette: black, charcoal, dark-teal)*
**Background:** Deeper recess of the cavern lit by an unseen pale teal-green glow, more rock formations receding into haze  *(palette: teal, pale-green, slate-gray, ice-blue)*
**Mood / lighting:** grim, backlit
**Emotion read:** ominous stillness, predatory silence
**Objects:** stalactites, stalagmites, cave-formations
**Iconography:** fanged-maw, shrine
**Genre cues:** fantasy, dark-fantasy

## Trivia

- **Set context** — Sanctum of Stone Fangs is part of the M21 Shrine cycle: five uncommon Legendary Enchantment — Shrine cards (one per color) plus one rare five-color Shrine, Sanctum of All. The cycle revived the Shrine subtype first printed in Champions of Kamigawa (CHK, 2004) as the Honden cycle — the only Shrines in existence until M21 sixteen years later. Sanctum of Stone Fangs is the black entry. `[Scryfall, WebSearch: dotesports.com — Shrines return to Magic: The Gathering in Core Set 2021]`
- **Mechanical reputation** — The drain effect (each opponent loses X life, you gain X life where X equals Shrines you control) scales in Shrine-tribal Commander decks. EDHREC rank 3,554 is by far the highest of the five uncommon Sanctums, appearing in approximately 39,000 Commander decks and roughly 96% of Shrine-focused Commander lists. It is the most-played of the M21 uncommon Sanctums. `[Scryfall, WebSearch: EDHREC — Sanctum of Stone Fangs]`
- **Flavor text** — Haiku format, no speaker: "Renounce all limits, / False chains binding potential. / Power knows no bounds." The three-line structure is a deliberate formal haiku — a nod to Kamigawa, the Japanese-mythology-inspired plane where the original Shrine subtype was created. `[Scryfall]`
- **Design** — Art by Johannes Voss, one of MTG's most prominent illustrators for enchantment-class cards. The composition is entirely landscape — no figures — depicting a stalactite-and-stalagmite cave as a fanged stone maw. The "fangs" of the title are literal geology rendered as predatory architecture. Voss uses a teal-green bioluminescent glow against near-black rock silhouettes as the sole light source. `[Scryfall]`
- **Design history** — The M21 Shrine cycle was designed by Adam Prosak's team as a callback to the original Honden cycle. Wizards commentary (Making Magic column, June 2020) confirmed the design challenge was making the new cycle feel like a genuine continuation. The haiku flavor-text format across the uncommon Sanctums was part of that continuity gesture. `[WebSearch: magic.wizards.com — But Wait, There's Core (2020-06-15)]`

### Related cards
- Sanctum of All (Core Set 2021) — five-color rare Shrine; the cycle's payoff card that supercharges all other Shrines
- Honden of Night's Reach (Champions of Kamigawa) — original black Shrine; the 2004 ancestor this card continues
- Sanctum of Calm Waters (Core Set 2021) — blue entry in the M21 Shrine cycle; design-family peer
- Go-Shintai of Life's Origin (Neon Dynasty Commander) — later Shrine commander that gives the M21 Sanctums a dedicated commander home
