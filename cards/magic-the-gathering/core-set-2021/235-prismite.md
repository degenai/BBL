---
name: Prismite
game: "Magic: The Gathering"
set: Core Set 2021
collector_number: 235
rarity: C
variance: Normal
grade: Ungraded
condition: Near Mint
quantity: 2
held_for_lair: 0
bundles: []
tags_hub:
  - crystal
  - golem
  - rainbow
  - prism
  - light
  - construct
  - gem
  - refraction
  - courtyard
  - mineral
tags_filter:
  - solo
  - mid-shot
  - action
  - forward-facing
  - artifact-creature
  - colorless
reference_image: cards/_images/magic-the-gathering/core-set-2021/235-prismite.png
average_cost_paid: 0
market_price: 0.1
market_price_as_of: 2026-05-27
date_added: 2026-05-17
last_seen: 2026-05-26
reference_image_source_url: https://cards.scryfall.io/png/front/e/e/eedce8ab-771a-4247-9504-72ae0629df83.png?1594737532
art_match_confidence: high
needs_manual_review: false
artist: Alayna Danner
art_crop_image: cards/_images/magic-the-gathering/core-set-2021/235-prismite--art.jpg
art_crop_source_url: https://cards.scryfall.io/art_crop/front/e/e/eedce8ab-771a-4247-9504-72ae0629df83.jpg?1594737532
flavor_text: Even its shadow is a dazzling rainbow.
oracle_text: "{2}: Add one mana of any color."
mana_cost: "{2}"
image_width: 745
image_height: 1040
image_quality: high
mood: action
time_of_day: day
setting: urban
---

# Prismite (Core Set 2021)

## Vision

![235-prismite](../../_images/magic-the-gathering/core-set-2021/235-prismite.png)

A humanoid golem made entirely of interlocking crystal facets crouches low in what appears to be a stone courtyard, weight forward, arms spread wide. The creature's surface refracts sunlight into scattered rainbow fragments that fall across the pavement beneath it. The crystal construction is dense and angular — there is no organic component visible, only the glittering, multi-faceted mineral body. Warm afternoon light behind and to the right deepens shadows in the crystal's crevices. Garden or architecture elements are faintly visible in the blurred background.

**Subject:** A crystal golem crouching in a courtyard, its entire body composed of faceted prismatic crystal shards that catch and refract light into rainbow scatters across the stone pavement.

**Composition:** mid-shot, action, figures: solo, facing: forward
**Setting:** urban, day, clear
**Foreground:** Crystal golem in crouched stance, prismatic light scattered on stone pavement  *(palette: silver, white, gold, prismatic rainbow)*
**Background:** Sunlit courtyard, soft architectural shapes and foliage  *(palette: warm tan, muted green, pale gold)*
**Mood / lighting:** action, harsh
**Emotion read:** Aggressive readiness — low crouching stance with arms splayed, weight forward
**Objects:** stone pavement, rainbow light scatter
**Creatures:** crystal golem
**Iconography:** prismatic rainbow, crystal facets
**Genre cues:** fantasy

## Trivia

- **Set context** — Prismite first appeared in War of the Spark (WAR, May 2019) as a colorless mana-fixer in a set dominated by multicolor Planeswalker cards; it was reprinted in Core Set 2020 (M20, July 2019) and again in Core Set 2021 (M21, July 2020). Three printings in three consecutive annual releases signals its value as a Limited and budget-casual mana-fixing utility. `[Scryfall prints search]`
- **Flavor text** — *"Even its shadow is a dazzling rainbow."* The flavor text is the entire card's design pitch compressed to eleven words: a creature whose material nature converts any input energy into full-spectrum output. The shadow detail is the precise visual joke — even the negative space it casts is prismatic. `[Scryfall]`
- **Design** — Prismite's oracle text is a single ability: `{2}: Add one mana of any color.` It is a 2/1 for two generic mana, requiring no colored mana to cast. This is the cleanest possible design for a colorless mana-filter: neutral body, neutral cost, pure rainbow output. The crystal-golem type is common visual grammar for artifact mana-fixers in MTG — compare Manalith, Pillar of Origins, and the Izzet Keyrune. `[Scryfall]`
- **Artist** — Alayna Danner has illustrated more than 50 Magic cards, first contributing in Hour of Devastation (2017). Hipsters of the Coast has covered her work in multiple pieces noting she "has become one of the game's current go-to artists for landscapes." Prismite is a departure from her landscape specialty — a creature portrait — but keeps her clean, luminous palette. `[Hipsters of the Coast: "Alayna Danner Sells Two Storybook Originals," 2019]`
- **Mechanical reputation** — EDHREC rank 13,835; sees play primarily in Pauper as a budget color-fixer in three-color lists that cannot reliably afford more expensive fixing. Legal in Pauper, Pioneer, Modern, Legacy, Vintage, and Commander. `[Scryfall]`

### Related cards
- Manalith (various) — 3-mana colorless mana rock with same function; the higher-cost predecessor Prismite partially replaced in some Pauper lists
- Sylvok Explorer (Mirrodin) — earlier Pauper mana-fixer with a creature body; same role, different era
- Chromatic Star (Time Spiral) — another cheap colorless mana-filter with a cantrip; shares the color-neutrality design space
