---
name: Jousting Dummy
game: Magic: The Gathering
set: Throne of Eldraine
collector_number: 224
rarity: C
variance: Normal
grade: Ungraded
condition: Near Mint
quantity: 1
held_for_lair: 0
bundles: []
tags_hub: ["scarecrow", "knight", "tournament", "jousting", "armor", "lance", "training", "effigy", "fairy-tale", "banners", "comic-relief", "medieval"]
tags_filter: ["solo", "mid-shot", "three-quarter", "portrait-mode", "no-face", "artifact", "creature-scarecrow", "colorless-magic", "day-scene"]
reference_image: images/magic-the-gathering/throne-of-eldraine/224-jousting-dummy.png
average_cost_paid: 0
market_price: 0.05
market_price_as_of: 2026-05-07
date_added: 2026-05-07
last_seen: 2026-05-07
art_match_confidence: high
needs_manual_review: false
manual_review_reason: No reference image found via scryfall for set 'Throne of Eldraine'
reference_image_source_url: https://cards.scryfall.io/png/front/f/6/f6601056-af08-4239-97d5-5e11597fce18.png?1572491020
mood: comedic
time_of_day: day
setting: other
---

# Jousting Dummy (Throne of Eldraine)

## Vision

![[images/magic-the-gathering/throne-of-eldraine/224-jousting-dummy.png]]

A weathered training dummy stands in a tournament field, dressed up as a mock knight: a kettle-style helmet adorned with green plumes sits atop a sackcloth-stitched scarecrow head, with painted features. Its body wears a battered metal cuirass and pauldrons over rough cloth, and one arm holds a wooden lance angled forward as if frozen mid-tilt. The dummy is fixed atop a sturdy wooden post or pivot, the classic medieval quintain. Behind it, the tournament grounds stretch out under daylight: colorful pennants and striped banners flutter from posts, and the silhouettes of castle structures and lists fill the background. Color palette is warm and earthy — browns, greens, soft golds — with the storybook fairy-tale feel characteristic of Throne of Eldraine.

**Subject:** A scarecrow-like jousting practice dummy (quintain) outfitted as a knight, with a helmeted head, armored torso, and lance, mounted on a wooden post in a tournament field

**Composition:** mid-shot, portrait, figures: solo, facing: three-quarter
**Setting:** other, day, clear
**Foreground:** armored scarecrow jousting dummy on a wooden post, holding a lance  *(palette: brown, tarnished-silver, olive-green, burlap-tan, rust)*
**Background:** tournament field with banners, pennants, and distant castle structures under daylight  *(palette: pale-blue, cream, muted-green, soft-gold, stone-grey)*
**Mood / lighting:** comedic, soft
**Emotion read:** inert, comically stoic — a mute mannequin playing knight
**Objects:** lance, helmet, cuirass, wooden-post, banners, pennants, armor
**Creatures:** scarecrow
**Iconography:** heraldry, tournament-banner, plume
**Genre cues:** fantasy, fairy-tale, medieval

