---
name: Ardenvale Tactician
game: Magic: The Gathering
set: Throne of Eldraine
collector_number: 5
rarity: C
variance: Normal
grade: Ungraded
condition: Near Mint
quantity: 1
held_for_lair: 0
bundles: []
tags_hub: ["knight", "griffin", "sword", "flight", "wings", "armor", "cavalry", "sky", "clouds", "heroism", "mounted-warrior", "chivalry"]
tags_filter: ["duo", "mid-shot", "faces-right", "scene-mode", "action-mode", "human-figure", "male-figure", "creature-knight", "white-magic", "flying", "adventure-mechanic"]
reference_image: images/magic-the-gathering/throne-of-eldraine/5-ardenvale-tactician.png
average_cost_paid: 0
market_price: 0.07
market_price_as_of: 2026-05-07
date_added: 2026-05-06
last_seen: 2026-05-07
art_match_confidence: high
needs_manual_review: false
manual_review_reason: No reference image found via scryfall for set 'Throne of Eldraine'
reference_image_source_url: https://cards.scryfall.io/png/front/c/7/c7d5e394-8e41-442e-ae97-a478a61e1b9d.png?1572489629
mood: sublime
time_of_day: magic-hour
setting: other
---

# Ardenvale Tactician (Throne of Eldraine)

## Vision

![[5-ardenvale-tactician.png]]

A heroic knight in burnished gold-and-silver plate armor sits astride a tawny griffin mid-flight, blade raised triumphantly toward the upper-right of the frame. A red cape or banner streams back from the rider, and the griffin's eagle forequarters and feathered wings catch warm light from a bright sky behind them. The composition is dynamic and upward-thrusting, with cloud-suffused atmosphere and a glow that suggests sunrise or magic-hour backlighting. The overall palette is warm gold, ivory, and rust against a pale luminous sky.

**Subject:** An armored knight raising a longsword aloft while mounted on a griffin in flight, banner-cape streaming behind

**Composition:** mid-shot, action, figures: duo, facing: right
**Setting:** other, magic-hour, clear
**Foreground:** armored knight on griffin, sword raised, cape flowing  *(palette: gold, ivory, rust-red, tawny-brown)*
**Background:** luminous cloud-filled sky with warm backlight  *(palette: pale-gold, cream, soft-white, peach)*
**Mood / lighting:** sublime, backlit
**Emotion read:** resolute, triumphant, charging forward
**Objects:** longsword, plate-armor, cape, banner, reins
**Creatures:** griffin
**Iconography:** raised-sword, heroic-charge, winged-mount
**Genre cues:** fantasy, high-fantasy, arthurian

