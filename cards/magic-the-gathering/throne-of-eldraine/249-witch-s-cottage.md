---
name: Witch's Cottage
game: Magic: The Gathering
set: Throne of Eldraine
collector_number: 249
rarity: C
variance: Normal
grade: Ungraded
condition: Near Mint
quantity: 3
held_for_lair: 0
bundles: []
tags_hub: ["cottage", "swamp", "willow", "witch", "twilight", "fairy-tale", "home", "lantern-light", "wetland", "fog", "storybook", "isolated-dwelling"]
tags_filter: ["no-figures", "wide-shot", "scene-mode", "no-face", "land", "swamp-land", "black-magic", "landscape-composition"]
reference_image: images/magic-the-gathering/throne-of-eldraine/249-witch-s-cottage.png
average_cost_paid: 0
market_price: 1.01
market_price_as_of: 2026-05-07
date_added: 2026-05-05
last_seen: 2026-05-07
reference_image_source_url: https://cards.scryfall.io/png/front/b/8/b87891cd-b457-4dff-8d18-a7eaf6748fc6.png?1572491253
art_match_confidence: high
needs_manual_review: false
mood: cozy
time_of_day: twilight
setting: forest
---

# Witch's Cottage (Throne of Eldraine)

## Vision

![[249-witch-s-cottage.png]]

A modest stone-and-thatch cottage sits on a small island or bank in the middle of a dim swamp, its arched windows and door radiating a warm amber-yellow lantern light against the surrounding gloom. Long, drooping branches of weeping willow trees hang heavily on either side, their tendrils touching the still, dark water. Faint mist rises from the surface and softens the silhouettes of distant trees. The sky behind is a deep violet-blue twilight, with no figures visible — the dwelling itself is the subject. The mood is hushed, fairy-tale, and slightly ominous, like a storybook home glimpsed at the edge of an enchanted wetland.

**Subject:** A small thatched cottage with warmly glowing yellow windows nestled at the edge of a dark swamp, framed by weeping willows trailing into still water under a twilight sky.

**Composition:** wide, narrative, figures: none, facing: n/a
**Setting:** forest, organic, twilight, fog
**Foreground:** Weeping willow branches, dark swamp water, the lit cottage on a small bank.  *(palette: warm amber, deep brown, muted olive-green, black)*
**Background:** Misty swamp receding into a violet-blue twilight sky with silhouetted trees.  *(palette: violet, indigo, muted teal, dusky blue)*
**Mood / lighting:** cozy, ambient
**Emotion read:** hushed, inviting yet uneasy — a warm light in a dark place
**Objects:** cottage, thatched-roof, lit-windows, lantern-light, willow-branches, swamp-water
**Iconography:** glowing-window, isolated-dwelling, weeping-willow
**Genre cues:** fantasy, fairy-tale, storybook

