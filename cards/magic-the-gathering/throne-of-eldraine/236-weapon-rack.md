---
name: Weapon Rack
game: Magic: The Gathering
set: Throne of Eldraine
collector_number: 236
rarity: C
variance: Normal
grade: Ungraded
condition: Near Mint
quantity: 3
held_for_lair: 0
bundles: []
tags_hub: ["weapons", "armory", "helm", "sword", "axe", "armor", "knight", "castle", "stonework", "medieval", "still-life", "blacksmith"]
tags_filter: ["no-figure", "no-face", "scene-mode", "mid-shot", "indoor", "artifact", "white-magic"]
reference_image: cards/_images/magic-the-gathering/throne-of-eldraine/236-weapon-rack.png
average_cost_paid: 0
market_price: 0.05
market_price_as_of: 2026-05-14
date_added: 2026-05-06
last_seen: 2026-05-13
reference_image_source_url: https://cards.scryfall.io/png/front/8/9/89ca22d2-3ba5-4173-9c8c-6587a901ff4a.png?1572491137
art_match_confidence: high
needs_manual_review: false
mood: other
time_of_day: indeterminate
setting: indoor
artist: Joe Slucher
art_crop_image: cards/_images/magic-the-gathering/throne-of-eldraine/236-weapon-rack--art.jpg
art_crop_source_url: https://cards.scryfall.io/art_crop/front/8/9/89ca22d2-3ba5-4173-9c8c-6587a901ff4a.jpg?1572491137
flavor_text: No weapon stays on the rack for long in the Burning Yard.
oracle_text: This artifact enters with three +1/+1 counters on it.\n{T}: Move a +1/+1 counter from this artifact onto target creature. Activate only as a sorcery.
---

# Weapon Rack (Throne of Eldraine)

## Vision

![236-weapon-rack](../../_images/magic-the-gathering/throne-of-eldraine/236-weapon-rack.png)

An indoor armory scene shows a heavy wooden rack fixed to a mortared stone wall. A knight's great helm crowned with a dark plume hangs prominently in the center, flanked by a longsword and what appears to be an axe or polearm hafted in dark wood. Smaller armor fittings and weapons are arranged across the rack's pegs. The lighting is warm and low, with an orange glow at the bottom edge suggesting torchlight or a hearth out of frame. No figures are present; the composition is a still life of arms.

**Subject:** A wooden weapon rack mounted against a stone wall, displaying a plumed great helm, a sword, an axe or polearm, and other arms and armor pieces in a castle armory.

**Composition:** mid-shot, narrative, figures: none, facing: n/a
**Setting:** indoor, gothic, indeterminate
**Foreground:** wooden weapon rack with plumed helm, sword, axe/polearm, and assorted arms and armor  *(palette: brown, iron-grey, black, warm-orange)*
**Background:** mortared stone wall of a castle interior  *(palette: grey, warm-brown, shadow-black)*
**Mood / lighting:** other, ambient
**Emotion read:** still, absent, awaiting use
**Objects:** weapon-rack, great-helm, plume, sword, axe, polearm, armor, wooden-pegs
**Iconography:** arms-and-armor, knightly-helm
**Genre cues:** fantasy, medieval

