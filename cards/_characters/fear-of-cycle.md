...
- Cycle membership at full canonical scope (every "Fear of" card Wizards printed in DSK and any Alchemy: Duskmourn or DSK Commander follow-ons) is not confirmed by this node — only the five corpus members are. A future triviabot pass on Fear of Abduction, Fear of Failed Tests, and Fear of Falling (currently vision-stub only in the corpus, tags_hub empty) will canonicalize the full cycle roster via MTG Wiki cross-reference. The node's `appears_on:` list expands as the corpus grows; this is the same growth pattern documented in the Bolas-Falls cluster character node.
- The cycle's color distribution across the corpus is white (1), blue (2), red (2). This does not preclude the canonical cycle from being a 1-per-color or 1-per-color-plus-multicolor structure at the full-set scope — it only reflects the partial-membership state of the BBL corpus on 2026-05-12. The two blue members (Failed Tests, Falling) and two red members (Being Hunted, Burning Alive) may reflect rarity-tiered or multi-card-per-color cycle structure that triviabot will canonicalize later.
