---
name: Paras
game: Pokemon
set: Sun & Moon Base Set
collector_number: 4
rarity: Common
variance: Normal
grade: Ungraded
condition: Near Mint
quantity: 1
held_for_lair: 0
bundles: []
tags_hub:
  - forest
  - mushroom
  - insect
  - nature
  - parasite
  - fungus
  - bug
  - cute
  - orange
  - evolution-line
tags_filter:
  - solo
  - mid-shot
  - three-quarter-facing
  - portrait-mode
reference_image: cards/_images/pokemon/sun-moon-base-set/4-paras.jpg
average_cost_paid: 0
market_price: 0.04
market_price_as_of: 2026-05-24
date_added: 2026-05-16
last_seen: 2026-05-23
reference_image_source_url: https://archives.bulbagarden.net/media/upload/2/20/ParasSunMoon4.jpg
art_match_confidence: high
needs_manual_review: false
manual_review_reason: Set 'Sun & Moon Base Set' did not match a known set code or the card was not in that set; fuzzy fallback returned art that may be from a different printing.
artist: Ayaka Yoshida
flavor_text: No matter how much it eats, the mushrooms growing on its back steal away most of the nutrients it consumes.
oracle_text: Scratch (GrassColorless) 30
image_width: 908
image_height: 1280
image_quality: high
mood: peaceful
time_of_day: day
setting: forest
suspected_ip: Paras
ip_confidence: high
ip_verified: false
characters:
  - paras-line
---

# Paras (Sun & Moon Base Set)

## Vision

![4-paras](../../_images/pokemon/sun-moon-base-set/4-paras.jpg)

> [!warning] Suspected IP: **Paras** (confidence: high, unverified)
> Reviewer: confirm whether the depicted figure is canonically this character. If yes, set `ip_verified: true` in frontmatter. If no, clear `suspected_ip`.

An orange, crab-like insect creature crouches on a leaf or low ground among lush green ferns and forest undergrowth. It has large, round, protruding blue eyes and a pair of small pincers or fangs visible at the front. Most visually striking are the two mushrooms growing from its back: one large red-capped mushroom with yellow spots and a smaller red-capped companion behind it. The surrounding environment is dense and green, with sunlight filtering through foliage in warm yellows. The overall composition is naturalistic and lush, evoking a forest floor ecosystem.

**Subject:** Paras, an orange crab-like insect Pokemon with large blue eyes and two tochukaso mushrooms growing from its back, sitting in a forest setting

**Composition:** mid-shot, portrait, figures: solo, facing: three-quarter
**Setting:** forest, day, calm
**Foreground:** Orange crab-insect creature with large blue eyes and two red-capped mushrooms on its back, crouching on a leaf  *(palette: orange, red, blue, green)*
**Background:** Dense green forest undergrowth with ferns, warm filtered sunlight  *(palette: green, yellow-green, warm-yellow)*
**Mood / lighting:** peaceful, soft
**Emotion read:** alert, curious wide-eyed expression
**Creatures:** insect, crab, mushroom-bearer
**Iconography:** mushroom
**Genre cues:** anime, fantasy, nature

## Connections

- [[paras-line]] — Base-stage Paras (National Pokedex no. 46) at one of the line's four corpus prints; the Ayaka Yoshida Sun & Moon Base Set art renders the canonical small orange crab-insect with two red-capped tochukaso mushrooms in a lush green forest-floor setting with dense ferns and filtered sunlight — the corpus's most pronounced naturalistic-forest-floor register for the species. The printed flavor *"No matter how much it eats, the mushrooms growing on its back steal away most of the nutrients it consumes"* is the species' canonical nutrient-theft-from-host Pokedex window, distinct from the DAB-003 "tochukaso are controlling the bug" flavor and staging an earlier phase of the parasitic-completion progression. Sibling Paras print to the Atsuko Nishida Darkness Ablaze 003/189 base-stage rendering, with the cross-set base-stage pair surfacing the line's designer-coordinated body-plan continuity across two generations of TCG production.

