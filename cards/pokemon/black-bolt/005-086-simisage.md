---
name: Simisage
game: Pokemon
set: Black Bolt
collector_number: 005/086
rarity: Uncommon
variance: Normal
grade: Ungraded
condition: Near Mint
quantity: 1
held_for_lair: 0
bundles: []
tags_hub:
  - monkey
  - primate
  - forest
  - jungle
  - plant-life
  - grass
  - spines
  - tropical
  - bad-temper
  - wild-animal
  - leaf
  - nature
tags_filter:
  - solo
  - mid-shot
  - three-quarter
  - portrait
  - uncommon
reference_image: cards/_images/pokemon/black-bolt/005-086-simisage.png
average_cost_paid: 0
market_price: 0.04
market_price_as_of: 2026-05-24
date_added: 2026-05-19
last_seen: 2026-05-23
reference_image_source_url: https://images.pokemontcg.io/zsv10pt5/5_hires.png
art_match_confidence: high
needs_manual_review: false
artist: kurumitsu
flavor_text: Ill tempered, it fights by swinging its barbed tail around wildly. The leaf growing on its head is very bitter.
oracle_text: Gentle Slap (ColorlessColorless) 70
image_width: 733
image_height: 1024
image_quality: high
mood: action
time_of_day: day
setting: forest
suspected_ip: Simisage
ip_confidence: high
ip_verified: false
characters:
  - elemental-monkey-trio
---

# Simisage (Black Bolt)

## Vision

![005-086-simisage](../../_images/pokemon/black-bolt/005-086-simisage.png)

> [!warning] Suspected IP: **Simisage** (confidence: high, unverified)
> Reviewer: confirm whether the depicted figure is canonically this character. If yes, set `ip_verified: true` in frontmatter. If no, clear `suspected_ip`.

A bipedal monkey creature stands upright in a lush green environment, its body primarily green with a jagged leaf-shaped growth on top of its head and a long spined or barbed tail curling behind it. Its posture suggests readiness or mild aggression — weight shifted, arms loosely out. The surrounding foliage is dense and tropical, with yellow orb-like fruit or seed pods visible nearby. The creature's expression carries an ill-tempered quality consistent with its flavor text. The overall palette is saturated green and yellow.

**Subject:** Simisage, a bipedal green monkey Pokemon with a leaf-topped head and barbed tail

**Composition:** mid-shot, portrait, figures: solo, facing: three-quarter
**Setting:** forest, day, calm
**Foreground:** green bipedal monkey creature with leaf head-growth and spined tail  *(palette: green, yellow-green, brown, yellow)*
**Background:** dense tropical foliage with yellow fruit  *(palette: green, yellow, dark green)*
**Mood / lighting:** action, ambient
**Emotion read:** ill-tempered, ready to lash out, tense body language
**Objects:** fruit, foliage
**Creatures:** monkey, primate, plant-creature
**Genre cues:** anime, fantasy

## Connections

- [[elemental-monkey-trio]] — Grass-arm evolved-form representative of the elemental monkey trio (Pansage / Pansear / Panpour and their Simi- evolved forms), the designer-coordinated Generation V Unova cohort of three paired Pokemon species mapped onto the three wise monkeys of Japanese cultural iconography (Pansage / *iwazaru* / speak no evil; Pansear / *kikazaru* / hear no evil; Panpour / *mizaru* / see no evil), each pair evolving via an elemental Stone rather than by level-up. This Black Bolt Simisage 005/086 is the corpus's second Simisage print (after Darkness Ablaze 007/189) and the cohort's first Black Bolt evolved-form member. Printed flavor *"Ill tempered, it fights by swinging its barbed tail around wildly. The leaf growing on its head is very bitter"* — the species' canonical aggressive-counterpoint-to-Pansage register, the deliberate designer-coordinated tonal inversion where the Grass arm's evolved form reverses the base form's docile leaf-as-soothing-remedy framing into a barbed-tail bitter-leaf temperament. Sits within the within-BLB adjacent-collector slot 005 immediately following Pansage 004/086, its base form — both Black Bolt prints illustrated by **kurumitsu**, a within-set single-artist-coordinated base-and-evolved pair-placement parallel to the Darkness Ablaze 006-007 Pansage/Simisage adjacency and the Black Bolt 017-018 Panpour/Simipour adjacency already in corpus. With this attachment the elemental-monkey-trio cohort node points to 13 corpus prints across 6 canonical species and 4 sets, and the Black Bolt Unova-retrospective coverage reaches four members (Pansage BB-004, Simisage BB-005, Panpour BB-017, Simipour BB-018). `[PokemonTCG.io: zsv10pt5-5 — flavor_text + artist; Bulbapedia: Simisage (Pokemon), Elemental monkeys design page]`
