---
name: Braviary
game: Pokemon
set: Black Bolt
collector_number: 078/086
rarity: Uncommon
variance: Normal
grade: Ungraded
condition: Near Mint
quantity: 4
held_for_lair: 0
bundles: []
tags_hub:
  - bird
  - flight
  - eagle
  - sky
  - freedom
  - warrior
  - canyon
  - wings
  - heroism
  - predator
tags_filter:
  - solo
  - wide-shot
  - forward-facing
  - uncommon
  - scene-mode
  - flying
  - pokemon-species
reference_image: cards/_images/pokemon/black-bolt/078-086-braviary.png
average_cost_paid: 0
market_price: 0.07
market_price_as_of: 2026-05-24
date_added: 2026-05-19
last_seen: 2026-05-23
reference_image_source_url: https://images.pokemontcg.io/zsv10pt5/78_hires.png
art_match_confidence: high
needs_manual_review: false
artist: Kedamahadaitai Yawarakai
flavor_text: They fight for their friends without any thought about danger to themselves. One can carry a car while flying.
oracle_text: "Aerial Ace (Colorless) 40+: Flip a coin. If heads, this attack does 40 more damage.\\nSpeed Wing (ColorlessColorlessColorlessColorless) 130"
image_width: 733
image_height: 1024
image_quality: high
mood: action
time_of_day: day
setting: mountain
suspected_ip: Braviary
ip_confidence: high
ip_verified: false
characters:
  - rufflet-line
---

# Braviary (Black Bolt)

## Vision

![078-086-braviary](../../_images/pokemon/black-bolt/078-086-braviary.png)

> [!warning] Suspected IP: **Braviary** (confidence: high, unverified)
> Reviewer: confirm whether the depicted figure is canonically this character. If yes, set `ip_verified: true` in frontmatter. If no, clear `suspected_ip`.

Braviary is captured in full-spread flight, wings extended to their full dramatic wingspan filling much of the frame. The bird's coloring is vivid — a red beak and facial plumage around a white head, dark navy-to-black wing feathers with blue-grey secondary feathers, and a broadly patterned chest. Below and behind it, a landscape of red-orange rocky cliffs or canyon formations is visible under a bright sky with scattered clouds. The composition is wide and action-oriented, emphasizing the creature's scale and aerial power. The mood is heroic and kinetic.

**Subject:** Braviary, a large eagle-like bird with a red, white, and blue color scheme, wings spread wide in dynamic flight

**Composition:** wide, action, figures: solo, facing: forward
**Setting:** mountain, day, clear
**Foreground:** Braviary with wings fully spread in active flight  *(palette: red, white, navy blue, dark grey)*
**Background:** red-orange rocky canyon landscape, bright sky with clouds  *(palette: red-orange, rust, pale blue, white)*
**Mood / lighting:** action, harsh
**Emotion read:** heroic, powerful, free
**Objects:** canyon walls, clouds
**Creatures:** Braviary, eagle, bird of prey
**Genre cues:** anime

## Connections

- [[rufflet-line]] — Apex-stage representative of the Rufflet evolutionary line (Pokedex 627 / 628, Normal/Flying, two-stage, male-exclusive, Unova Gen V). This Black Bolt Braviary 078/086 is the corpus's BLB-era Braviary print, illustrated by Kedamahadaitai Yawarakai and slotted at the within-set adjacent-collector-number pair partner to Rufflet 077/086 by the same artist — the BLB set's fourth within-set adjacent-collector-number single-artist-coordinated evolution-line pair alongside Snivy 001 + Servine 002 (Susumu Maeya), Dwebble 051 + Crustle 052 (Tetsu Kayama), and Pawniard 064 + Bisharp 065 (Oku). Printed flavor *"They fight for their friends without any thought about danger to themselves. One can carry a car while flying"* — the species' canonical friend-protecting-hero temperament rendered as printed Pokedex flavor, the apex-stage counterpart to the Rufflet prints' fearless-aspirant flavor. The wide canyon flight composition with red-orange cliffs is a deliberate Americana iconography render (red/white/blue plumage against canyon-landscape Unova-as-American-Southwest geographic register), at the community-attested designer-intent scope per the cohort node's body. `[PokemonTCG.io: zsv10pt5-78; Bulbapedia: Braviary (Pokemon)]`

