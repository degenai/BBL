---
name: Galarian Farfetch'd
game: Pokemon
set: Rebel Clash
collector_number: 094/192
rarity: Common
variance: Normal
grade: Ungraded
condition: Near Mint
quantity: 2
held_for_lair: 0
bundles: []
characters: ["galarian-regional-forms"]
tags_hub: ["bird", "warrior", "fighter", "leek", "weapon", "galar", "regional-form", "martial", "brave", "duck", "outdoors"]
tags_filter: ["solo", "mid-shot", "action", "three-quarter", "fighting-magic", "basic-pokemon"]
reference_image: cards/_images/pokemon/rebel-clash/094-192-galarian-farfetch-d.png
average_cost_paid: 0
market_price: 0.13
market_price_as_of: 2026-05-13
date_added: 2026-05-08
last_seen: 2026-05-12
reference_image_source_url: https://images.pokemontcg.io/swsh2/94_hires.png
art_match_confidence: high
needs_manual_review: false
artist: Akira Komayama
flavor_text: The Farfetch'd of the Galar region are brave warriors, and they wield thick, tough leeks in battle.
oracle_text: Rock Smash (Fighting) 10+: Flip a coin. If heads, this attack does 30 more damage.\nPierce (ColorlessColorless) 30
mood: action
time_of_day: day
setting: mountain
suspected_ip: Galarian Farfetch'd (Pokémon Sword and Shield)
ip_confidence: high
ip_verified: true
---

# Galarian Farfetch'd (Rebel Clash)

## Vision

![094-192-galarian-farfetch-d](../../_images/pokemon/rebel-clash/094-192-galarian-farfetch-d.png)

> [!note] IP verified: **Galarian Farfetch'd (Pokémon Sword and Shield)**
> Confirmed via PokemonTCG.io card data (swsh2-94), card name, Fighting-type mapping to Galarian form, and exact Pokedex flavor text match (brave warriors, thick leeks).

A duck-like bird creature with dark grey-black feathers and a white chest stands in an assertive fighting stance. The creature holds a thick leek stalk as a weapon, angled forward as if ready to strike. The background is an open daytime sky with light clouds and rocky terrain below. The posture and prop convey determination and martial readiness. The color contrast between the dark feathered body and the bright green leek is visually distinctive.

**Subject:** Galarian Farfetch'd, a dark-feathered bird Pokémon, wielding a leek stalk in a fighting stance

**Composition:** mid-shot, action, figures: solo, facing: three-quarter
**Setting:** mountain, day, clear
**Foreground:** dark-feathered bird creature in fighting stance holding a leek  *(palette: dark-grey, white, green)*
**Background:** open sky with light clouds, rocky terrain  *(palette: light-blue, white, grey)*
**Mood / lighting:** action, ambient
**Emotion read:** determined, combative, ready to fight
**Objects:** leek, plant-stalk-weapon
**Creatures:** farfetch-d, bird
**Genre cues:** anime

## Trivia

- **Set context** — Galarian Farfetch'd appears in Sword & Shield — Rebel Clash (expansion 2, released May 1, 2020 in English). It is classified as a Fighting-type Basic Pokémon, reflecting the Galarian form's Fighting typing as opposed to the Kanto original's Normal/Flying typing [PokemonTCG.io: swsh2-94; Bulbapedia: Rebel Clash (TCG)].
- **Regional form lore** — The Galarian form is a pure Fighting type. In-game lore frames Galarian Farfetch'd as warrior-class fighters who carry thick, tough leeks as weapons. The oversized leek is a deliberate design choice: leeks native to the United Kingdom (the real-world basis for Galar) grow significantly larger than leeks in Japan [PokéJungle: Galarian Farfetch'd and Sirfetch'd — Origin of Species].
- **Evolution mechanic** — Galarian Farfetch'd evolves into Sirfetch'd after landing exactly 3 critical hits in a single battle — one of the more unusual in-battle evolution conditions in the series. The evolution can trigger even if Galarian Farfetch'd loses the battle. On evolution, the leek splits: the leafy top becomes a shield, the stem a lance, referencing medieval weapons [Bulbapedia: Sirfetch'd; The Gamer: how to evolve Galarian Farfetch'd].
- **Flavor text** — *"The Farfetch'd of the Galar region are brave warriors, and they wield thick, tough leeks in battle."* `[PokemonTCG.io: swsh2-94]`

### Related cards
- Sirfetch'd (various Sword & Shield era prints) — evolution target, same Galarian form line
- Farfetch'd (original Kanto form, various sets) — the Normal/Flying predecessor; the contrast between forms anchors the regional-forms lore
- Leek (item card, if in TCG) — the held item that boosts critical hit ratio for the Farfetch'd line

## Connections

- [[115-172-farfetch-d]] — Regional-variant contrast pair. Galarian Farfetch'd (Fighting-type, dark-grey plumage, leek-as-weapon in a martial action pose, mountain setting) is the Galar-region variant of Kanto Farfetch'd (Normal/Flying, brown-and-cream plumage, leek-carried-as-prop in a pastoral painterly countryside). The contrast is designer-coordinated and load-bearing for the [[galarian-regional-forms]] node's design thesis: Galar's tougher real-world leeks plus local warrior tradition redesigned the species into a Fighting-type that *uses* the leek as a weapon rather than carrying it as a vestigial item. `[Bulbapedia: Galarian Farfetch'd — Origin and Pokedex entries, via WebSearch]` `[PokeJungle: Galarian Farfetch'd and Sirfetch'd — Origin of Species, via WebSearch]` `[[galarian-regional-forms]]`
