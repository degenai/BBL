---
name: Galarian Darumaka
game: Pokemon
set: Rebel Clash
collector_number: 047/192
rarity: Common
variance: Normal
grade: Ungraded
condition: Near Mint
quantity: 2
held_for_lair: 0
bundles: []
characters: ["galarian-regional-forms"]
tags_hub: ["winter", "snow", "ice", "cold", "round", "creature", "cute", "galar", "regional-form", "tundra", "stillness"]
tags_filter: ["solo", "mid-shot", "portrait", "forward-facing", "water-magic", "basic-pokemon"]
reference_image: cards/_images/pokemon/rebel-clash/047-192-galarian-darumaka.png
average_cost_paid: 0
market_price: 0.05
market_price_as_of: 2026-05-14
date_added: 2026-05-08
last_seen: 2026-05-14
reference_image_source_url: https://images.pokemontcg.io/swsh2/47_hires.png
art_match_confidence: high
needs_manual_review: false
artist: Akira Komayama
flavor_text: It lived in snowy areas for so long that its fire sac cooled off and atrophied. It now has an organ that generates cold instead.
oracle_text: Ice Punch (WaterColorless) 30: Flip a coin. If heads, your opponent's Active Pokémon is now Paralyzed.
mood: peaceful
time_of_day: day
setting: mountain
suspected_ip: Galarian Darumaka (Pokémon Sword and Shield)
ip_confidence: high
ip_verified: true
---

# Galarian Darumaka (Rebel Clash)

## Vision

![047-192-galarian-darumaka](../../_images/pokemon/rebel-clash/047-192-galarian-darumaka.png)

> [!note] IP verified: **Galarian Darumaka (Pokémon Sword and Shield)**
> Confirmed via PokemonTCG.io card data (swsh2-47), card name, Water-type mapping to Ice-type Galarian form, and exact Pokedex flavor text match (fire sac atrophy lore).

A small, rotund creature with a white and light-blue rounded body stands in a snowy outdoor environment. The creature's body is predominantly white and spherical, with teal-green markings visible on its head and small dark eyes. The background features a cold, overcast sky and a snow-covered ground, establishing a wintry setting. The overall tone is cool and icy, and the creature's compact form gives it a cheerful, plush-toy quality.

**Subject:** Galarian Darumaka, a round ice-type Pokémon, in a snowy winter landscape

**Composition:** mid-shot, portrait, figures: solo, facing: forward
**Setting:** mountain, day, snow
**Foreground:** round white and teal creature standing in snow  *(palette: white, teal, light-blue)*
**Background:** snowy ground, overcast sky  *(palette: light-grey, white, pale-blue)*
**Mood / lighting:** peaceful, soft
**Emotion read:** neutral, calm, compact and still
**Creatures:** darumaka, ice-type-creature
**Genre cues:** anime

## Trivia

- **Set context** — Galarian Darumaka appears in Sword & Shield — Rebel Clash (expansion 2, released May 1, 2020 in English), the same set where the Galarian regional form debuted in the TCG alongside other Galar-region variants [PokemonTCG.io: swsh2-47; Bulbapedia: Rebel Clash (TCG)].
- **Regional form lore** — The Galarian form adapts the original Unova Fire-type Darumaka (National Pokedex no. 554) into a pure Ice type. In-game lore explains the shift: Darumaka in Galar lived in snowy regions so long that their fire sac cooled off and atrophied, replaced by an organ that generates cold instead [PokemonTCG.io flavor text; Bulbapedia: Darumaka (Pokémon)].
- **Evolution and version exclusivity** — Galarian Darumaka evolves into Galarian Darmanitan via Ice Stone. Galarian Darmanitan's Zen Mode is notable as the first Ice/Fire dual-type Pokémon in the series. Galarian Darumaka is a Pokémon Sword version exclusive — not available in Shield without trading [Bulbapedia: Darumaka (Pokémon); PokemonDatabase.net: Sword/Shield exclusives].
- **Flavor text** — *"It lived in snowy areas for so long that its fire sac cooled off and atrophied. It now has an organ that generates cold instead."* `[PokemonTCG.io: swsh2-47]`

### Related cards
- Galarian Darmanitan (Rebel Clash) — evolution target via Ice Stone; same set
- Galarian Darmanitan (Sword & Shield base set or later prints) — alternate prints of the evolution line
- Darumaka (original Unova form) — the Fire-type predecessor whose regional divergence defines this form's lore
